...
This is the same MITM proxy `wrap --transport http` runs, plus enforcement: only a severity-3 `tools/call` proved against an *observed* declared surface for that HTTP session is refused, answered locally with the same `-32000` JSON-RPC error shape (marked `http_gate_blocked` rather than the stdio gate's `gate_blocked`, so the two enforcement paths stay distinguishable in a log) — no upstream connection is even opened for that request. Every case with missing *evidence* — no declaration yet seen, a malformed or still-paginating declaration, a faulted detector pass, a lost or stale session epoch — still forwards, with the would-block recorded for later replay. A request the gate cannot *read* is different: gate mode refuses it before any upstream connection rather than forward bytes it never analysed — `413` for a body over the inspection limit (4 MiB), `415` for a `Content-Encoding`, `400` for anything that is not exactly one unambiguous JSON object (batch, duplicate keys, BOM, invalid UTF-8, trailing data), and a `tools/call` whose id is missing or not a string/integer (invalid MCP input, recorded as `invalid_mcp_request`, distinct from a policy block) — and `403` for a request whose `Host` or `Origin` is not the loopback proxy itself (DNS rebinding). `observe` and `wrap` keep forwarding all of these byte-for-byte. A declaration is authority only while it is current: a server's `notifications/tools/list_changed`, or an elapsed `ttlMs` on its `tools/list` (measured on the wire clock, so replay agrees), makes the surface unknown until the client lists again — calls then forward rather than risk blocking a newly added tool. (The stdio gate keeps its own surface and does not yet do this.) This is intentionally narrow: it is not a general HTTP firewall, and PII, unexpected-egress, and other non-fabrication findings never block, over HTTP or stdio.
