...
decisions; it is not wired into either transport. HTTP enforcement remains
future work. See [the API, bounds, validation, and implementation record](docs/priority-engineering.md).
...
- HTTP enforcement parity: incremental built-in detectors and a separate policy interface are implemented in this source checkout, with batch/live parity tests. HTTP session routing, delivery/action recording, and enforcement integration remain. Existing `gate` is stdio-only and has not been migrated to the new engine.
