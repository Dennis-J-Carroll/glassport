...
class TestTaxonomyTags(unittest.TestCase):
    def test_secret_hardcoded_carries_asi_and_maestro_tags(self):
        report = Report(
            ...
            findings=[Finding(rule="secret-hardcoded", severity="critical",
                              path="src/x.py", line=1, detail="")],
            deductions=[{"rule": "secret-hardcoded", "severity": "critical",
                        "points": 25, "hits": 1}],
            score=75, grade="C")
        doc = json.loads(sarif.render_sarif(report))
        rule = next(r for r in doc["runs"][0]["tool"]["driver"]["rules"]
                    if r["id"] == "glassport/secret-hardcoded")
        props = rule["properties"]
        self.assertIn("asi", props)
        self.assertIn("maestro_layer", props)

    def test_untagged_rule_omits_taxonomy_keys_cleanly(self):
        report = Report(
            profile={"path": ".", "runtime": "unknown", "package_name": "",
                     "version": "", "dependency_count": 0, "files_scanned": 0,
                     "depth": {"ast": 0, "pattern": 0}},
            findings=[Finding(rule="no-license", severity="info",
                              path="LICENSE", line=1, detail="")],
            deductions=[], score=100, grade="A")
        doc = json.loads(sarif.render_sarif(report))
        rule = next(r for r in doc["runs"][0]["tool"]["driver"]["rules"]
                    if r["id"] == "glassport/no-license")
